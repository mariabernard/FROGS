...
```{r import}
# Import input files
abundances_input <- get(load(params$phyloseq_rdata))
deseq_rdata_input     <- get(load(params$deseq_rdata))
base_name <- dirname(params$deseq_rdata)
run_ipath <- ifelse(params$analysis == "ASV", FALSE, TRUE)
result_names <- DESeq2::resultsNames(deseq_rdata_input)

# doc <- ""
# if(length(result_names) == 2 || params$mod1 == "None"){
#      results <- DESeq2::results(deseq_rdata, name = result_names[1])
# } else if(length(result_names)>=3){
#      results <- DESeq2::results(deseq_rdata_input, contrast = c(params$varExp, params$mod1, params$mod2) )
...
abundances_input2 <- prune_samples(get_variable(abundances_input, params$varExp) %in% c(params$mod1, params$mod2), abundances_input)
...
if ( length(result_names)==2 || params$mod1 != "None"){
    ...
    if ( params$mod1!= "None" ){
    	#abundances_input <- prune_samples(get_variable(abundances_input, params$varExp) %in% c(params$mod1, params$mod2), abundances_input)
        subtitle <- paste(sep="", params$varExp, "_", params$mod1, "_vs_", params$mod2)
    }
	...
	hplot <- plot_heatmap(hplot_input, 
				...
				facet_grid(as.formula(paste("~", params$varExp)), scales = "free_x") + 
				ggtitle("Heatmap plot of DA asv or functions, between 2 conditions", subtitle = subtitle)
## Quantitative variable
} else {
    idx <- order(get_variable(abundances_input, params$varExp))
    ...
    hplot <- plot_heatmap(hplot_input, 
             ...
             scale_y_discrete(y_title) +
             ggtitle(paste0("Heatmap plot of DA asv or functions with samples ordered according to ", params$varExp))
}
# ggplotly(hplot)
hplot
```
